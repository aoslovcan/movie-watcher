...
const MovieCard = ({ id, title, posterPath, data }: MovieCardType) => {
  ...
  const [addToLocalStorage] = useStorage({ id, title, posterPath }, "favorites", "Already in favorites" );
  ...
  return (
      ...
      <div role="button" tabIndex={0} className="card" onClick={(e) => getMovieDetails(e)}>
        <BsFillBookmarkFill className="favourites-button" onClick={(e) => addToFavourite(e)} />
